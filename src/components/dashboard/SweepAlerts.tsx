      ...
      <div className="space-y-2 max-h-64 overflow-y-auto">
          ...
          >
            ...
            <div className="mt-2 text-sm">
              <div className="text-white font-bold">
                {alert.size} contratos {alert.symbol}
              </div>
              <div className="text-yellow-200">
                @ ${alert.price.toFixed(2)} on {alert.exchange}
              </div>
              <div className={`text-xs mt-1 ${
                alert.side === 'BUY' ? 'text-green-400' : 'text-red-400'
              }`}>
                {alert.side === 'BUY' ? '🟢 AGGRESSIVE BUY' : '🔴 AGGRESSIVE SELL'}
              </div>
            </div>
          </div>
        ))}
        ...
        {sweepAlerts.length === 0 && (
          <div className="text-center py-8">
            ...
            <p className="text-gray-400">No sweep alerts yet</p>
            <p className="text-xs text-gray-500 mt-1">
              Alerts will appear when large sweeps are detected
            </p>
          </div>
        )}
      </div>
